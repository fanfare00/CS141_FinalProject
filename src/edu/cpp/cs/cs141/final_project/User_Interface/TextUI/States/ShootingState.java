...
public class ShootingState extends TextUIState {
	public ShootingState(Application app) {
		keys.add(new Key('W', "W - Move UP", new ShootCommand(app, 1, 0)));
		keys.add(new Key('A', "A - Move LEFT", new ShootCommand(app, 0, -1)));
		keys.add(new Key('S', "S - Move DOWN", new ShootCommand(app, -1, 0)));
		keys.add(new Key('D', "D - Move RIGHT", new ShootCommand(app, 0, 1)));
		keys.add(new Key('C', "C - Look", new ToggleLookCommand(app)));
		keys.add(new Key('Z', "Z - Move", new ToggleMoveCommand(app)));
